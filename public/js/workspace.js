document.addEventListener('DOMContentLoaded',()=>{
  ...
  const select=group=>{selected?.classList.remove('is-selected');selected=group;selected?.classList.add('is-selected');selectedEmpty.hidden=Boolean(selected);selectedFields.hidden=!selected;if(selected){selectedName.textContent=selected.querySelector('.aisle-code')?.textContent||'Aisle';render(selected)}};
