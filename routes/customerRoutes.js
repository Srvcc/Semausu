...
async function storeMap(storeId,{includeProducts=true}={}){
  ...
  const products=rawProducts.map(product=>{
    ...
    if(product.aisle_id&&product.fixture_width){
  ...
  return{aisles,sections,entrances,products};
}
...
router.post('/api/supermarkets/:slug/route',async(req,res)=>{
  ...
  const map=await storeMap(supermarket.id),ids=Array.isArray(req.body.productIds)?req.body.productIds.map(String):[],start=map.entrances.find(x=>x.id===String(req.body.entranceId))||map.entrances[0],finish=map.entrances.find(x=>x.id===String(req.body.finishId))||start;
  ...
  for(const destination of destinations){const points=findPath(current,destination,map.aisles,supermarket.map_width,supermarket.map_height);if(!points.length)return res.status(422).json({error:'No walkable path connects every stop. The store should check corridor gaps and fixture positions.'});segments.push({from:{x:number(current.x),y:number(current.y)},to:{x:number(destination.x),y:number(destination.y)},points});current=destination}
